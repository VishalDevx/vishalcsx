...
const NODE_STYLES: Record<string, string> = {
  client:  "color:#60a5fa;background:rgba(59,130,246,0.08);borderColor:rgba(59,130,246,0.2)",
  gateway: "color:#a78bfa;background:rgba(167,139,250,0.08);borderColor:rgba(167,139,250,0.2)",
  service: "color:#34d399;background:rgba(52,211,153,0.08);borderColor:rgba(52,211,153,0.2)",
  infra:   "color:#fbbf24;background:rgba(251,191,36,0.08);borderColor:rgba(251,191,36,0.2)",
  db:      "color:#f87171;background:rgba(248,113,113,0.08);borderColor:rgba(248,113,113,0.2)",
};
...
function Mono({ children, color }: { children: React.ReactNode; color?: string }) {
  return (
    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: color ?? "rgba(167,139,250,0.85)" }}>
      {children}
    </span>
  );
}
...
function SectionLabel({ text }: { text: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
      <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, letterSpacing: "0.22em", textTransform: "uppercase", color: "#3f3f46", whiteSpace: "nowrap" }}>
        {text}
      </span>
...
function ArchDiagram({ rows, colors }: { rows: string[][]; colors: string[][] }) {
  return (
    <div style={{ padding: "22px 28px", borderTop: "0.5px solid rgba(255,255,255,0.07)", background: "#09090b" }}>
      <p style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.18em", textTransform: "uppercase", color: "#3f3f46", marginBottom: 14 }}>
        Architecture flow
      </p>
      {rows.map((row, ri) => (
        <div key={ri} style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6, paddingLeft: ri === 1 ? 160 : 0 }}>
          {row.map((node, ni) => (
            <>
              ...
              {ni < row.length - 1 && (
                <span key={`arrow-${ni}`} style={{ color: "#3f3f46", fontSize: 12 }}>→</span>
              )}
            </>
          ))}
        </div>
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginTop: 8 }}>
      <thead>
        <tr>
          {["Decision", "Chosen", "Rejected"].map((h) => (
            <th key={h} style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "#3f3f46", padding: "8px 10px", textAlign: "left", borderBottom: "0.5px solid rgba(255,255,255,0.07)" }}>
              {h}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={r.decision}>
            <td style={{ padding: "9px 10px", borderBottom: "0.5px solid rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.5)", fontFamily: "'DM Mono', monospace", fontSize: 10, whiteSpace: "nowrap", verticalAlign: "top" }}>
              {r.decision}
            </td>
            <td style={{ padding: "9px 10px", borderBottom: "0.5px solid rgba(255,255,255,0.04)", verticalAlign: "top" }}>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.08em", padding: "2px 7px", borderRadius: 3, color: "#34d399", background: "rgba(52,211,153,0.1)", border: "0.5px solid rgba(52,211,153,0.2)" }}>
                {r.chosen}
              </span>
            </td>
            <td style={{ padding: "9px 10px", borderBottom: "0.5px solid rgba(255,255,255,0.04)", verticalAlign: "top" }}>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.08em", padding: "2px 7px", borderRadius: 3, color: "#f87171", background: "rgba(248,113,113,0.07)", border: "0.5px solid rgba(248,113,113,0.15)" }}>
                {r.rejected}
              </span>
            </td>
          </tr>
...
export default function SystemsPage() {
  ...
  return (
    <>
      ...
      <main style={{ minHeight: "100vh", background: "#09090b", color: "#fafafa", fontFamily: "'DM Sans', sans-serif" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 32px 80px" }}>

          {/* ── HEADER ── */}
          <div style={{ padding: "64px 0 48px", borderBottom: "0.5px solid rgba(255,255,255,0.07)", marginBottom: 60 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18, fontFamily: "'DM Mono', monospace", fontSize: 10, letterSpacing: "0.22em", textTransform: "uppercase", color: "#3f3f46" }}>
              <span style={{ width: 20, height: "0.5px", background: "#3f3f46", display: "block" }} />
              Engineering Systems
            </div>
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(44px,6vw,76px)", fontWeight: 800, lineHeight: 0.92, letterSpacing: "-0.035em", color: "#fafafa" }}>
              How I{" "}
              <span style={{ WebkitTextStroke: "1.2px rgba(255,255,255,0.2)", color: "transparent" }}>
                architect
              </span>
              <br />
              and ship systems
            </h1>
            <p style={{ marginTop: 20, maxWidth: 520, fontSize: 15, color: "#71717a", fontWeight: 300, lineHeight: 1.75 }}>
            ...
            <div style={{ marginTop: 28, display: "flex", flexWrap: "wrap", gap: 8 }}>
              {["Multi-tenant architecture", "Distributed systems", "Production-grade APIs", "Redis caching", "PostgreSQL at scale"].map((t) => (
                <span key={t} style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", padding: "5px 12px", borderRadius: 4, border: "0.5px solid rgba(255,255,255,0.11)", color: "#71717a", background: "#18181b" }}>
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="principles-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1px", background: "rgba(255,255,255,0.07)", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden", marginBottom: 72 }}>
            {PRINCIPLES.map((p) => (
              <div key={p.num} style={{ background: "#09090b", padding: 24 }}>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#3f3f46", letterSpacing: "0.1em", marginBottom: 14 }}>{p.num}</div>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 16, fontWeight: 700, color: "#fafafa", marginBottom: 8, letterSpacing: "-0.01em" }}>{p.title}</div>
                <div style={{ fontSize: 13, color: "#71717a", lineHeight: 1.65, fontWeight: 300 }}>{p.desc}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 72 }}>
            {CASE_STUDIES.map((cs) => {
              ...
              return (
                <article key={cs.id} style={{ border: `0.5px solid ${isOpen ? "rgba(255,255,255,0.11)" : "rgba(255,255,255,0.07)"}`, borderRadius: 20, overflow: "hidden", transition: "border-color 0.15s" }}>

                  {/* Header */}
                  <div
                    onClick={() => toggle(cs.id)}
                    style={{ padding: "28px 32px 24px", borderBottom: "0.5px solid rgba(255,255,255,0.07)", display: "grid", gridTemplateColumns: "1fr auto", alignItems: "start", gap: 16, cursor: "pointer", background: isOpen ? "#111113" : "#09090b", transition: "background 0.15s" }}
                  >
                    <div>
                      <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: "#60a5fa", background: "rgba(59,130,246,0.1)", border: "0.5px solid rgba(59,130,246,0.22)", borderRadius: 3, padding: "3px 8px", display: "inline-block", marginBottom: 12 }}>
                        {cs.tag}
                      </span>
                      <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 22, fontWeight: 700, color: "#fafafa", letterSpacing: "-0.02em", marginBottom: 6 }}>{cs.title}</h2>
                      <p style={{ fontSize: 13, color: "#71717a", fontWeight: 300 }}>{cs.sub}</p>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 14 }}>
                        {cs.stack.map((s) => (
                          <span key={s} style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#71717a", background: "#18181b", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 3, padding: "2px 7px" }}>{s}</span>
                        ))}
                      </div>
                    </div>
                    <button style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: "#3f3f46", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 4, padding: "6px 12px", background: "none", cursor: "pointer", flexShrink: 0, marginTop: 4 }}>
                  ...
                  {isOpen && (
                    <>
                      ...
                      {/* 4 panels */}
                      <div className="cs-panel-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "rgba(255,255,255,0.07)" }}>
                        {cs.panels.map((panel) => (
                          <div key={panel.label} style={{ background: "#09090b", padding: "22px 26px" }}>
                            <p style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.18em", textTransform: "uppercase", color: "#3f3f46", marginBottom: 10 }}>{panel.label}</p>
                            <p style={{ fontSize: 13, color: "#71717a", lineHeight: 1.7, fontWeight: 300 }}>
                      ...
                      {/* Decision table */}
                      <div style={{ background: "#09090b", padding: "22px 26px", borderTop: "0.5px solid rgba(255,255,255,0.07)" }}>
                        <p style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.18em", textTransform: "uppercase", color: "#3f3f46", marginBottom: 10 }}>Key decisions</p>
                        <DecisionTable rows={cs.decisions} />
                      </div>

                      {/* Metrics */}
                      <div className="metrics-grid" style={{ display: "grid", gridTemplateColumns: `repeat(${cs.metrics.length},1fr)`, gap: "1px", background: "rgba(255,255,255,0.07)", borderTop: "0.5px solid rgba(255,255,255,0.07)" }}>
                        {cs.metrics.map((m) => (
                          <div key={m.label} style={{ background: "#09090b", padding: "18px 20px", textAlign: "center" }}>
                            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 26, fontWeight: 700, color: "#fafafa", lineHeight: 1 }}>{m.val}</div>
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.14em", textTransform: "uppercase", color: "#3f3f46", marginTop: 5 }}>{m.label}</div>
                          </div>
                        ))}
                      </div>
                    </>
          ...
          <div className="radar-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1px", background: "rgba(255,255,255,0.07)", border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden", marginBottom: 72 }}>
            {Object.entries(TECH_RADAR).map(([zone, items]) => (
              <div key={zone} style={{ background: "#09090b", padding: "20px 18px" }}>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", color: "#3f3f46", marginBottom: 14, paddingBottom: 10, borderBottom: "0.5px solid rgba(255,255,255,0.07)" }}>
                  <span style={{ color: RADAR_COLORS[zone] }}>●</span>{" "}{zone}
                </div>
                {items.map((item) => (
                  <div key={item} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 9, fontSize: 12, color: "#71717a", fontFamily: "'DM Mono', monospace" }}>
          ...
          <div className="scale-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {SCALE_PATTERNS.map((p) => (
              <div key={p.title} style={{ border: "0.5px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "22px 24px", background: "#09090b", transition: "border-color 0.15s" }}>
                ...
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 15, fontWeight: 700, color: "#fafafa", marginBottom: 8, letterSpacing: "-0.01em" }}>{p.title}</div>
                <div style={{ fontSize: 13, color: "#71717a", lineHeight: 1.65, fontWeight: 300 }}>{p.desc}</div>
              </div>
            ))}
          </div>
