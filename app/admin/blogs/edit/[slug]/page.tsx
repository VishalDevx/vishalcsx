...
export default function EditBlog() {
  ...
  const [form, setForm] = useState({
    title: "", category: "Backend", type: "Deep Dive", excerpt: "", content: "",
    image: "", readTime: "5 min read", tags: "", featured: false, published: true,
  });
    ...
    fetch("/api/admin/blogs")
      ...
      .then((blogs) => {
        ...
        if (blog) {
          setForm({
            title: blog.title || "",
            category: blog.category || "Backend",
            type: blog.type || "Deep Dive",
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-6 flex items-center gap-4">
        <Link href="/admin/blogs" className="flex h-8 w-8 items-center justify-center rounded-lg border transition-all hover:bg-[var(--card-hover)]" style={{ borderColor: "var(--border-color)", color: "var(--text-secondary)" }}>
          <ArrowLeft size={16} />
        </Link>
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--text-primary)" }}>Edit Blog Post</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <label className="mb-1.5 block text-xs font-medium" style={{ color: "var(--text-secondary)" }}>Content</label>
