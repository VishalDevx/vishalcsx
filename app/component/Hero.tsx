...
const iconMap: Record<string, any> = { BsGithub, BsLinkedin, BsTwitter, SiGmail };
...
export function Hero() {
  ...
  const publishedArticles = blogs?.filter((b) => b.published).slice(0, 2) || [];

  if (!profile) {
    return (
      <section className="flex min-h-screen items-center justify-center pt-14" style={{ backgroundColor: "var(--bg-primary)" }}>
        <div className="h-8 w-8 animate-spin rounded-full border-2 border-[var(--accent)] border-t-transparent" />
      </section>
    );
  }

  const SocialIcon = (name: string) => iconMap[name] || null;
      ...
      <section style={{ fontFamily: "'Space Grotesk', sans-serif", backgroundColor: "var(--bg-primary)", color: "var(--text-primary)", transition: "background-color 0.3s ease, color 0.3s ease" }} className="relative overflow-hidden pt-14">
        ...
        <div className="relative z-10 mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-3 border-b py-4 sm:flex-row sm:items-center sm:justify-between sm:py-6" style={{ borderColor: "var(--border-subtle)" }}>
            <span style={{ fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--text-muted)", ...mono }}>{profile.domain || "vishal.dev"}</span>
            {profile.available && (
            ...
            <div className="flex min-w-0 flex-col gap-6 sm:gap-7 lg:gap-8">
              <div className="hero-fade lg:hidden" style={{ animationDelay: "0.04s" }}>
                <div className="overflow-hidden rounded-2xl border" style={{ borderColor: "var(--card-border)", backgroundColor: "var(--bg-secondary)" }}>
                  <div className="relative aspect-[4/3] w-full sm:aspect-[16/10]">
                    <Image src="https://avatars.githubusercontent.com/VishalDevx" alt={profile.name || "Vishal Singh"} fill className="object-cover grayscale transition-all duration-500 hover:grayscale-0" priority />
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                      <p style={{ fontSize: "10px", letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(161,161,170,0.8)", ...mono }}>{profile.name} — {profile.location}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="hero-fade" style={{ animationDelay: "0.14s" }}>
                <h1 className="text-[clamp(40px,15vw,96px)] leading-[0.92] tracking-[-0.05em]" style={{ ...heading, fontWeight: 800, color: "var(--text-primary)" }}>
                  ...
                  <span className="text-transparent" style={{ WebkitTextStroke: "1.2px var(--stroke)" }}>{profile.name?.split(" ").slice(1).join(" ") || "Singh"}</span>
                </h1>
              </div>
              ...
              <div className="hero-fade" style={{ animationDelay: "0.2s" }}>
                <p className="max-w-[620px] text-lg font-light leading-relaxed sm:text-xl md:text-2xl" style={{ color: "var(--text-secondary)" }}>
                  {profile.heroDescription || "I design & build scalable systems and tackle complex backend challenges"}
                </p>
              </div>
              ...
              <div className="hero-fade grid w-full max-w-[640px] grid-cols-1 overflow-hidden rounded-xl border sm:grid-cols-3" style={{ borderColor: "var(--card-border)", animationDelay: "0.32s" }}>
                {profile.stats?.map((s: any, i: number) => (
                  <div key={s.label} className={`px-5 py-4 transition-colors hover:bg-[var(--card-hover)] sm:px-6 md:px-7 ${i < profile.stats.length - 1 ? "border-b sm:border-b-0 sm:border-r" : ""}`} style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--card-border)" }}>
              ...
              <div className="hero-fade flex flex-wrap items-center gap-2" style={{ animationDelay: "0.44s" }}>
                {profile.socialLinks?.map((s: any) => {
                  ...
                  if (!Icon) return null;
                  return (
                    <Link key={s.label} href={s.url} target="_blank" aria-label={s.label} className="flex h-10 w-10 items-center justify-center rounded-md border transition-all hover:text-[var(--text-primary)]" style={{ borderColor: "var(--border-color)", color: "var(--icon-color)" }}>
                      <Icon size={15} />
                    </Link>
                  );
                })}
            ...
            <div className="hero-fade hidden flex-col gap-4 lg:flex" style={{ animationDelay: "0.18s" }}>
              ...
              <div className="rounded-xl border p-4" style={{ borderColor: "var(--card-border)", backgroundColor: "var(--card-bg)" }}>
                ...
                <div className="flex flex-wrap gap-[6px]">
                  {profile.coreStack?.map((tech: string) => (
              ...
              <div className="flex items-center justify-between rounded-xl border p-4" style={{ borderColor: "var(--card-border)", backgroundColor: "var(--card-bg)" }}>
                <div>
                  ...
                  <p className="text-sm font-light" style={{ color: "var(--text-primary)" }}>{profile.availabilityText || "Open to full-time & freelance"}</p>
                </div>
          ...
          <div className="pb-16 sm:pb-20 lg:pb-24">
            <div className="mb-6 flex flex-col gap-3 sm:mb-8 sm:flex-row sm:items-center sm:gap-4">
              ...
              <Link href="/blogs" className="w-fit whitespace-nowrap rounded border px-3 py-[6px] text-[10px] uppercase tracking-[0.1em] transition-all hover:text-[var(--text-primary)]" style={{ borderColor: "var(--border-color)", color: "var(--text-secondary)", ...mono }}>
            ...
            <div className="grid gap-px overflow-hidden rounded-xl border md:grid-cols-2" style={{ borderColor: "var(--card-border)", backgroundColor: "var(--card-border)" }}>
              {publishedArticles.length > 0 ? publishedArticles.map((post: any) => (
                <article key={post.slug} className="group cursor-pointer p-5 transition-colors hover:bg-[var(--card-hover)] sm:p-6 md:p-7" style={{ backgroundColor: "var(--card-bg)" }}>
                ...
                <div className="p-8 text-center sm:p-10" style={{ backgroundColor: "var(--card-bg)" }}>
